...
//        writeClassToFile(clazz);
    ...
    private MethodSpec createMethodWithElements(Set<? extends Element> elements) {
        ...
        builder.addStatement("$T<$T> set = new $T<>()", Set.class, String.class, HashSet.class);
...
//            ClassType type = (ClassType) element.asType();
            ...
            builder.addStatement("set.add($S)", typeElement.getQualifiedName());
        }
    ...
    private TypeSpec createClassWithMethod(MethodSpec method) {
        // 定义一个名字叫 OurClass 的类
        TypeSpec.Builder ourClass = TypeSpec.classBuilder("DemoAptClass");

        // 声明为 public 的
        ourClass.addModifiers(Modifier.PUBLIC);

        // 为这个类加入一段注释
        ourClass.addJavadoc("这个类是通过 javapoet 创建的类\n\n@author Jay\n");
    ...
    private void writeClassToFile(TypeSpec clazz) {
        ...
        JavaFile file = JavaFile.builder("com.jay.aptdemo", clazz).build();
